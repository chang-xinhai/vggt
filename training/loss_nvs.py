...
class NovelViewSynthesisLoss(nn.Module):
    ...
    Replaces the original implementation with LVSM-based loss.
    """
    
    def __init__(self, rgb=None, perceptual=None, lpips_loss=None, **kwargs):
        super().__init__()
        self.config = edict()
        self.config.training = edict()
        
        # Default values
        self.config.training.l2_loss_weight = 1.0
        self.config.training.lpips_loss_weight = 0.0
        self.config.training.perceptual_loss_weight = 0.5
        
        if rgb is not None:
            # We use the rgb weight for l2 loss
            self.config.training.l2_loss_weight = rgb.get('weight', 1.0)
            
        if perceptual is not None:
             self.config.training.perceptual_loss_weight = perceptual.get('weight', 0.0)
             
        # Allow configuring lpips via kwargs or a new config section if added
        if lpips_loss is not None:
             self.config.training.lpips_loss_weight = lpips_loss.get('weight', 0.0)
    ...
    def forward(self, predictions, batch):
        ...
        """
        rendering = predictions['rgb']  # (B, S_out, H, W, 3)
        target = batch['target_images']  # (B, S_out, 3, H, W)
        
        # Permute rendering to (B, S_out, 3, H, W) to match target and LVSM expectation
        rendering = rendering.permute(0, 1, 4, 2, 3)
        ...
        # Return dict matching expected output format
        loss_dict = {
            'loss_rgb': l2_loss,
            'loss_psnr': psnr,
