...
def normalize_scene():
    """
    Normalize the scene by centering and scaling all objects to fit within [-1, 1]^3.
    Following GS-LRM: "center and scale each 3D object to a bounding box of [−1, 1]^3"
    """
    ...
    for obj in mesh_objects:
        for vertex in obj.bound_box:
            ...
            bbox_min = np.minimum(bbox_min, np.array(world_coord))
            bbox_max = np.maximum(bbox_max, np.array(world_coord))
    
    # Calculate center and scale
    center = (bbox_min + bbox_max) / 2
    scale = 2.0 / np.max(bbox_max - bbox_min)
